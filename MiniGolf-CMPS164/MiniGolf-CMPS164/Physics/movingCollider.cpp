...
RigidSphere::RigidSphere() : Rigidbody(), Sphere(){
	radius = 1;
}

RigidSphere::RigidSphere(float r) : Rigidbody(), Sphere(r) {
	radius = r;
}

RigidSphere::RigidSphere(float r, Force f) : Rigidbody(f), Sphere(r) {
	radius = r;
}

RigidSphere::RigidSphere(float r, Position startLoc) : Rigidbody(startLoc), Sphere(startLoc, r){
	radius = r;
}

RigidSphere::RigidSphere(float r, Position startLoc, Force f) : Rigidbody(startLoc, f), Sphere(startLoc, r) {
	radius = r;
}
